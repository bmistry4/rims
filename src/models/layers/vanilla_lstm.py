"""
Use this file instead of rims.py with a VanillaLSTM cell. It will just act as a way of doing an LSTM with all the
RIMs stuff (e.g. sparsity, masking, attn ...) stripped away.
"""
import torch
...
class VanillaLSTM(nn.Module):
    ...
    def __init__(self, cell_name: str, d_enc_inp, d_hid_all_rims_per_layer=[600], num_rims_per_layer=[6],
                 num_active_rims_per_layer=[4], use_comm_attn=False, num_modules_read_input=2, in_num_head=1,
                 comm_num_head=4,
        ...
        for i in range(len(num_rims_per_layer)):
            ...
            # fixme - after the 1st layer, a layer's number of key/values will match the number of output rims of the prev layer?
            in_num_k_or_v = num_modules_read_input  # +1 to consider the null input k/v

            # create cell for the layer
            cell = str_to_class(cell_name)(num_rims=num_rims_per_layer[i], d_input=d_enc_inp,
                                           d_hidden=d_hid_all_rims_per_layer[i], device=self.device)
            print(f"Cell type used: {cell._get_name()}\n")

            # todo - testing vanillaLSTM; reset to line below once done
            self.rims_cells.append(cell)
            # self.rims_cells.append(RIMsCell(cell, d_cell_inp_single_rim, d_hid_all_rims_per_layer[i],
            #                                 num_rims_per_layer[i], num_masked_rims_per_layer[i],
            #                                 True, in_num_k_or_v, in_num_head, comm_num_head,
            #                                 in_d_k, comm_d_k,
            #                                 comm_d_v,
    ...
    def forward(self, input: TensorType["timestep", "batch", "d_enc_inp"]) -> \
        ...
        for l_idx in range(num_layers):
            ...
            for ts_idx in range(input.shape[0]):
                # todo - testing vanillaLSTM; remove if stmt once done
                if states is None:
                    states = rims_cell.init_states(bsz=input.shape[1])

                # todo - testing vanillaLSTM; remove if stmt once done
                # add 'number' dimension to the input (which will represent the number of keys/values)
                if len(layer_input[ts_idx].shape) == 3:
                    inp_use = layer_input[ts_idx].reshape(
                        (layer_input[ts_idx].shape[0], layer_input[ts_idx].shape[2]))  # [B,d_enc_in] -> [B,n_blk_in=1, d_enc_inp]
                else:
                    inp_use = layer_input[ts_idx]

                # mask = [B, num_rims, d_rim]
                states = rims_cell(inp_use, states)  # todo - testing vanillaLSTM; reset to line below once done
                # states, mask = rims_cell(layer_input[ts_idx], states)

                # [B, num_rims, d_rim]
                hx = rims_cell.get_hidden_state(states) # todo - testing vanillaLSTM; reset to line below once done
                # hx = rims_cell.cell.get_hidden_state(states)

                # mask = [B, n_r, d_r]  # todo - testing vanillaLSTM; remove once done
                mask = torch.ones(hx.shape, requires_grad=False, device=self.device).detach()
                ...
                if l_idx == len(self.rims_cells) - 1:
                    ...
                    # do not allow inactive rims to contribution to the output
                        ...
                        if self.block_grads:
                            ...
                            output.append(hx)  # todo - testing vanillaLSTM; reset to line below once done
